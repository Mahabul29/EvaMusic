...
function _npOpenQueueSheet() {
    ...
    if (!sheet) {
        ...
        sheet.innerHTML = `
<style>
.np-queue-sheet{background:#1e1e1e;width:100%;max-width:600px;border-radius:20px 20px 0 0;padding:0;transform:translateY(100%);transition:transform .35s ease;box-sizing:border-box;max-height:85vh;display:flex;flex-direction:column}
...
.np-queue-scroll{overflow-y:auto;padding:8px 16px 20px;flex:1}
...
async function _npLoadQueueSongs() {
    ...
    if (artist) endpoints.push(`/api/search?q=${encodeURIComponent(artist)}&limit=20`);
    if (album) endpoints.push(`/api/search?q=${encodeURIComponent(album)}&limit=20`);
