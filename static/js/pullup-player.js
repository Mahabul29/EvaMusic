...
class EvaPlayer {
    ...
    async _autoExtendQueue() {
        ...
        const endpoints = [
            artist ? `/api/search?q=${encodeURIComponent(artist)}` : null,
            `/api/trending`,
            title  ? `/api/search?q=${encodeURIComponent(title.split(' ')[0])}` : null,
            `/api/search?q=latest`,
        ].filter(Boolean);

        let newSongs = [];
        for (const url of endpoints) {
            if (newSongs.length >= 5) break;
            try {
...
async function _npLoadQueueSongs() {
    ...
    let allSongs = [];
    // Use title+artist as unique key to filter duplicates (same song, different albums)
    ...
    const endpoints = [];
    if (artist) {
        endpoints.push(`/api/search?q=${encodeURIComponent(artist)}`);
        const artists = artist.split(/[,&]/).map(a => a.trim()).filter(a => a.length > 0);
        artists.slice(0, 3).forEach(a => {
            endpoints.push(`/api/search?q=${encodeURIComponent(a)}`);
        });
    }
    if (album) endpoints.push(`/api/search?q=${encodeURIComponent(album)}`);
    if (title) endpoints.push(`/api/search?q=${encodeURIComponent(title.split(' ')[0])}`);
    endpoints.push('/api/trending');
    endpoints.push('/api/search?q=latest');
    endpoints.push('/api/search?q=new');
    ...
    // Auto-populate player queue with related songs so Next/Skip works
    if (p && allSongs.length > 0) {
        // Keep current song at index 0, add related songs after
        const currentIdx = p.currentIndex;
        const currentInQueue = p.queue[currentIdx];
        const currentQueueId = currentInQueue ? p.getId(currentInQueue) : null;

        // Only update if the queue doesn't already have these songs
        const existingIds = new Set(p.queue.map(s => p.getId(s)));
        const newSongs = allSongs.filter(s => !existingIds.has(p.getId(s)));
