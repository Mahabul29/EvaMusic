...
async function _npLoadQueueSongs() {
    ...
    // Fetch related songs — max 15, 2 targeted endpoints only
    ...
    // Primary artist only (first name before comma), then trending as fallback
    const endpoints = [];
    if (artist) endpoints.push(`/api/search?q=${encodeURIComponent(artist.split(/[,&]/)[0].trim())}`);
    ...
    for (const url of endpoints) {
        ...
        try {
            ...
            const data = await res.json();
            let results = Array.isArray(data) ? data
                        : data.songs || data.data || data.results || [];
