...
class EvaPlayer {
    ...
    getImageUrl(song) {
        const DEFAULT = '/static/images/default-album.png';
        // Blocklist: URLs that belong to the app UI, not to songs
        const BLOCKED = ['/static/icon-512.png', '/static/icon-192.png', '/static/images/default-album.png'];
        ...
        if (!img || typeof img !== 'string') return DEFAULT;
        // Block profile avatar and app icons bleeding into music bar
        if (BLOCKED.includes(img)) return DEFAULT;
        if (img.includes('profileAvatar') || img.includes('profile-avatar')) return DEFAULT;
        return img;
    }
    ...
    refreshQueueFromDOM() {
        ...
        items.forEach(item => {
            ...
            // Skip profile/avatar elements that are not song cards
            if (item.dataset.profileOnly) return;
            if (id) {
                newQueue.push({
                    id:     id,
    ...
    next() {
        ...
        if (this.currentIndex < this.queue.length - 1) {
            ...
            // ── FIX: auto-extend instead of dead-stopping with "End of queue" toast ──
            this._autoExtendQueue();
        }
    }
    ...
    handleEnded() {
        if (this.repeatMode === 2) {
            ...
            // More songs in queue — play next
            this.next();
        } else if (this.repeatMode === 1) {
            // Repeat all — loop back to start
            this.currentIndex = 0; this.playSong(this.getId(this.queue[0]), this.queue[0]);
        } else {
            // ── FIX: Queue exhausted — auto-fetch related songs and continue ──
            // Old code: this.isPlaying = false; this._syncPlayIcons(false);
            // That caused the "End of queue" dead-stop the user reported.
            this._autoExtendQueue();
        }
    }

    // ── Auto-extend: fetch related songs when queue runs out ──────────────────
    async _autoExtendQueue() {
        const song = this.currentSong;
        if (!song) { this.isPlaying = false; this._syncPlayIcons(false); return; }

        showToast('Loading more songs…', 'info');

        const artist = this.getArtist(song);
        const title  = this.getTitle(song);
        const seenIds = new Set(this.queue.map(s => this.getId(s)));

        // Try endpoints in order until we get at least one new song
        // Helper: unwrap any JioSaavn API response shape
        const _extract = (data) => {
            if (!data) return [];
        ...
        const titleKeyword  = title  ? title.split(' ').slice(0, 2).join(' ') : '';
        const endpoints = [
            primaryArtist ? `/api/search?q=${encodeURIComponent(primaryArtist)}&limit=20` : null,
            titleKeyword  ? `/api/search?q=${encodeURIComponent(titleKeyword)}&limit=20`  : null,
        ...
        for (const url of endpoints) {
            ...
            } catch (e) { /* network error — try next endpoint */ }
        }

        if (newSongs.length === 0) {
            // Nothing found — fall back to looping the existing queue
            showToast('🔁 Restarting queue', 'info');
            this.currentIndex = 0;
            this.playSong(this.getId(this.queue[0]), this.queue[0]);
            return;
        }

        // Append new songs and play the first one
        this.queue = [...this.queue, ...newSongs];
        this.currentIndex++;  // points to first newly-added song
        const next = this.queue[this.currentIndex];
        this.playSong(this.getId(next), next);
    }
    ...
    // ── Skip forward/backward by the duration chosen in Settings → Playback ──
    _getSkipDuration() {
        try {
            const v = JSON.parse(localStorage.getItem('evamusic_skipDuration'));
    ...
    skipForward(seconds) {
        if (!this.audio.duration || isNaN(this.audio.duration)) return;
        const amt = seconds || this._getSkipDuration();
        this.audio.currentTime = Math.min(this.audio.duration, this.audio.currentTime + amt);
    }

    skipBackward(seconds) {
        const amt = seconds || this._getSkipDuration();
        this.audio.currentTime = Math.max(0, this.audio.currentTime - amt);
    }
    ...
    _updateBarUI(song) {
        ...
        const detail   = document.getElementById('musicBarDetail');
        if (thumb) {
        ...
        // Artist — make it a tappable link to artist search
        if (artist) {
            const artistName = this.getArtist(song);
            const primaryArtist = artistName.split(/[,&]/)[0].trim();
            const artistUrl = `/artist/${encodeURIComponent(primaryArtist)}`;
            artist.innerHTML = '';
            ...
            a.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (window._spaNavigateTo) window._spaNavigateTo(artistUrl);
                else window.location.href = artistUrl;
            });
            artist.appendChild(a);
        }

        // Detail line — album · duration
        if (detail) {
            const parts = [];
    ...
    // ── Media Session API — system notification panel ─────────────────────────
    _updateMediaSession(song) {
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
            ...
            album:  song.album || song.album_name || 'EvaMusic',
            artwork: [
        ...
        console.log('[EvaPlayer] MediaSession set:', this.getTitle(song), '|', this.getArtist(song));
    }
    ...
    _initMediaSession() {
        ...
        navigator.mediaSession.setActionHandler('seekbackward', (d) => {
            this.audio.currentTime = Math.max(0, this.audio.currentTime - (d.seekOffset || this._getSkipDuration()));
        });
        navigator.mediaSession.setActionHandler('seekforward', (d) => {
            this.audio.currentTime = Math.min(this.audio.duration || 0, this.audio.currentTime + (d.seekOffset || this._getSkipDuration()));
        });
        navigator.mediaSession.setActionHandler('seekto', (d) => {
            if (d.seekTime != null && this.audio.duration)
        ...
        console.log('[EvaPlayer] MediaSession handlers registered');
    }
    ...
    _updateOverlayUI(song) {
        ...
        const detail   = document.getElementById('npSongDetail');
        const imgUrl   = this.getImageUrl(song);
        if (art) {
            art.src = imgUrl;
        ...
        // Artist link → /artist/PrimaryArtist (real Artist Profile page)
        if (link) {
            const primaryArtist = this.getArtist(song).split(/[,&]/)[0].trim();
            const artistUrl = `/artist/${encodeURIComponent(primaryArtist)}`;
            link.href = artistUrl;
        ...
        // Keep skip-duration button labels in sync with Settings → Playback
        const skipSecs  = this._getSkipDuration();
        const backLabel = document.getElementById('npSkipBackLabel');
        const fwdLabel  = document.getElementById('npSkipFwdLabel');
        if (backLabel) backLabel.textContent = `${skipSecs}s`;
        if (fwdLabel)  fwdLabel.textContent  = `${skipSecs}s`;
    }
...
#npAlbumArt{width:100%;aspect-ratio:1/1;border-radius:16px !important;object-fit:cover;box-shadow:0 16px 48px rgba(0,0,0,0.7);display:block;margin:8px auto 22px;flex-shrink:0;transition:transform 0.3s ease}
...
.np-artist-row{display:flex;align-items:center;gap:6px;color:rgba(255,255,255,0.5);font-size:13px;cursor:pointer;-webkit-tap-highlight-color:transparent}
.np-artist-row:active{color:rgba(255,255,255,0.85)}
...
.np-skip-row{display:flex;justify-content:center;align-items:center;gap:56px;margin:-6px 0 16px}
.np-skip-btn{background:none;border:none;color:rgba(255,255,255,0.65);font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:6px;-webkit-tap-highlight-color:transparent;opacity:0.9;transition:opacity .15s}
.np-skip-btn:active{opacity:0.4}
.np-skip-btn i{font-size:15px}
...
/* Pull-up handle — swipe up (or tap) to open Queue */
.np-pullup-handle{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:44px;padding:14px 0 12px;color:rgba(255,255,255,0.35);font-size:18px;cursor:pointer;-webkit-tap-highlight-color:transparent;touch-action:none}
.np-pullup-handle i{animation:npPullUpBounce 1.8s ease-in-out infinite}
@keyframes npPullUpBounce{0%,100%{transform:translateY(0);opacity:0.35}50%{transform:translateY(-5px);opacity:0.85}}
</style>
...
<div id="npBg">
  ...
  <div id="npScroll">
    ...
    <div class="np-meta">
      <div class="np-meta-text">
        ...
        <div class="np-artist-row">
          ...
          <a id="npViewArtist" href="#" style="color:inherit;text-decoration:none;min-width:0;overflow:hidden;flex:1">
            <span id="npArtistLabel"></span>
          </a>
        </div>
        <div id="npSongDetail" style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:0.1px"></div>
      </div>
    ...
    <div class="np-skip-row">
      <button class="np-skip-btn" onclick="window.player.skipBackward()">
        <i class="fas fa-undo"></i><span id="npSkipBackLabel">10s</span>
      </button>
      <button class="np-skip-btn" onclick="window.player.skipForward()">
        <span id="npSkipFwdLabel">10s</span><i class="fas fa-redo"></i>
      </button>
    </div>
    ...
    <!-- Pull-up handle: swipe up (or tap) to open Queue -->
    <div class="np-pullup-handle" id="npPullUpHandle">
      <i class="fas fa-chevron-up"></i>
        ...
        const pullUpHandle = document.getElementById('npPullUpHandle');
        if (pullUpHandle) _npInitPullUpHandle(pullUpHandle);
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
// Pull-up handle — swipe up (or tap) on the bottom arrow to open the Queue
// ═══════════════════════════════════════════════════════════════════════════════

function _npInitPullUpHandle(handle) {
    if (handle._pullUpInit) return;
    handle._pullUpInit = true;

    let startY = 0;
    let tracking = false;
    const THRESHOLD = 28; // px swiped upward before triggering
    ...
    handle.addEventListener('touchend', () => { tracking = false; });
}
...
.np-queue-scroll{overflow-y:auto;padding:8px 16px 20px;flex:1;-webkit-overflow-scrolling:touch;max-height:70vh}
...
async function _npLoadQueueSongs() {
    ...
    const title = p.getTitle(current);
    ...
    // Fetch related songs — max 15, artist-targeted search
    const MAX_RELATED = 15;
    let allSongs = [];
    const seenKeys = new Set();
    const currentTitle  = p.getTitle(current).toLowerCase().trim();
    const currentArtist = p.getArtist(current).toLowerCase().trim();
    seenKeys.add(currentTitle + '|' + currentArtist);

    // Helper: extract songs array from any API response shape
    function _extractSongs(data) {
        if (!data) return [];
        if (Array.isArray(data)) return data;
        // JioSaavn API wraps in data.results or data.data.results
        if (data.data) {
            const d = data.data;
            if (Array.isArray(d)) return d;
            if (Array.isArray(d.results)) return d.results;
    ...
    // 3 targeted endpoints in priority order:
    // 1. Primary artist name (before any comma)
    // 2. Song title keywords (first 2 words)
    // 3. Trending as fallback only
    const primaryArtist = artist ? artist.split(/[,&]/)[0].trim() : '';
    const titleKeyword  = title  ? title.split(' ').slice(0, 2).join(' ') : '';
    const endpoints = [];
    if (primaryArtist) endpoints.push(`/api/search?q=${encodeURIComponent(primaryArtist)}&limit=20`);
    if (titleKeyword)  endpoints.push(`/api/search?q=${encodeURIComponent(titleKeyword)}&limit=20`);
    endpoints.push('/api/trending?limit=20');

    for (const url of endpoints) {
        if (allSongs.length >= MAX_RELATED) break;
        try {
            const res = await fetch(url);
            if (!res.ok) continue;
            const data = await res.json();
            const results = _extractSongs(data);

            for (const song of results) {
                if (allSongs.length >= MAX_RELATED) break;
                const songTitle  = p.getTitle(song).toLowerCase().trim();
                const songArtist = p.getArtist(song).toLowerCase().trim();
        ...
        } catch(e) { console.warn('[Queue] Fetch failed:', url, e); }
    }
    ...
    let listHtml = npHtml + `<div style="font-size:11px;font-weight:600;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.7px;padding:8px 4px 6px">Up Next <span style="color:rgba(255,255,255,0.2);font-weight:500">(${allSongs.length} songs)</span></div>`;
    ...
    _queueSheetSongs = allSongs;

    // Auto-populate player queue — max 15 new songs only
    if (p && allSongs.length > 0) {
        const existingIds = new Set(p.queue.map(s => p.getId(s)));
        const newSongs = allSongs
            .filter(s => !existingIds.has(p.getId(s)))
            .slice(0, 15);  // hard cap — never dump 150 songs into queue
        if (newSongs.length > 0) {
            p.queue = [...p.queue, ...newSongs];
        }
    }
}
...
function _npPlayQueueSong(songId, index) {
    ...
    // Add ALL remaining queue songs to player queue so Next button works
    const remainingSongs = _queueSheetSongs.slice(index + 1);
    p.queue = [song, ...remainingSongs];
    p.currentIndex = 0;
