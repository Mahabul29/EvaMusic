...
# JioSaavn language values: hindi, english, punjabi, tamil, telugu,
# marathi, gujarati, bengali, kannada, malayalam, urdu, etc.

def _detect_song_language(song):
    """Detect language from JioSaavn song data."""
    # 1. Check explicit language field (most reliable)
    lang = song.get('language') or song.get('lang') or ''
    if lang:
        return lang.lower().strip()

    # 2. Check artist name against known mappings
    artist = (song.get('artist') or song.get('primaryArtists') or 
              song.get('singers') or '').lower()

    # 3. Check song title for language clues
    title = (song.get('title') or song.get('name') or '').lower()

    # Common indicators in titles
    if any(x in title for x in ['punjabi', 'bhangra']): return 'punjabi'
    if any(x in title for x in ['tamil', 'kollywood']): return 'tamil'
    if any(x in title for x in ['telugu', 'tollywood']): return 'telugu'
    if any(x in title for x in ['bengali', 'bangla']): return 'bengali'
    if any(x in title for x in ['marathi']): return 'marathi'
    if any(x in title for x in ['gujarati', 'gujju']): return 'gujarati'
    if any(x in title for x in ['kannada', 'sandalwood']): return 'kannada'
    if any(x in title for x in ['malayalam', 'mollywood']): return 'malayalam'
    if any(x in title for x in ['urdu', 'ghazal', 'qawwali']): return 'urdu'

    # Default for Indian music platform
    return 'hindi'
...
def _filter_by_language(songs, lang):
    ...
    if len(filtered) < 5 and len(songs) > len(filtered):
        seen_ids = {str(s.get('id') or s.get('song_id', '')) for s in filtered}
        for song in songs:
            sid = str(song.get('id') or song.get('song_id', ''))
            if sid not in seen_ids:
                filtered.append(song)
            if len(filtered) >= 20:
                break

    print(f"[FILTER] lang={lang}: {len(songs)} -> {len(filtered)} songs")
    return filtered
...
def _extract_artists_from_songs(songs, lang=None):
    ...
    for song in songs:
        ...
        for name in artist_names:
            ...
            # Detect language from song
            song_lang = _detect_song_language(song)

            # Filter by language if specified
            if lang and lang.lower() != 'all':
                if song_lang != lang.lower():
                    continue
...
def _normalize_song(data):
    ...
    # Get language from JioSaavn data
    language = (inner.get("language") or inner.get("lang") or "hindi").lower()
...
def fetch_trending(limit=20, lang=None):
    ...
    # Fetch more songs to have enough after filtering
...
def fetch_song(song_id):
    ...
    print(f"[fetch_song] OK: {song['title']} | url={bool(song['url'])} | lang={song.get('language', 'unknown')}")
    return song
...
def home():
    ...
    # Taste summary for the template
...
def api_artists():
    """Get artists based on user taste + language filter."""
    user_id = get_user_id()
    lang = request.args.get('lang', 'All')
    ...
    for artist_name, weight in taste.get('top_artists', [])[:limit*2]:
        ...
        # For taste-based artists, we don't know their language
        # So we only include them if no language filter is applied
        # or if they match known language mappings
        if lang and lang.lower() != 'all':
            # Skip taste-based artists when language filter is on
            # because we can't reliably detect their language
            continue
    ...
    try:
        ...
        for item in favs:
            ...
            for name in names:
                ...
                # Check if item has language info
                item_lang = (item.get('language') or '').lower()
                if lang and lang.lower() != 'all':
                    if item_lang and item_lang != lang.lower():
                        continue
    ...
    # 4. Fallback to trending artists filtered by language
    if len(artists) < limit:
        print(f"[ARTIST] Only found {len(artists)} taste artists, fetching trending...")
    ...
    print(f"[API] Returning {len(artists)} artists")
