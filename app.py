...
def profile_page():
    ...
    playlists = []  # playlist feature not yet implemented server-side

    # Derive a "Favourite Artists" list from the artists on favorited songs.
    seen, favourite_artists = set(), []
    for song in favs:
        raw_artist = (song.get("artist") or "").strip()
        if not raw_artist:
            continue
        primary = raw_artist.split(",")[0].split("&")[0].strip()
        if not primary or primary.lower() in seen:
            continue
        seen.add(primary.lower())
        favourite_artists.append({
            "name":  primary,
            "image": song.get("image_url") or "/static/images/default-album.png",
        })
    ...
    return render_template(
        ...
        profile={
            ...
            "username":     (session.get('user_email',  '') or 'user').split('@')[0],
        ...
        stats={
            ...
            "listening_hours":  round(len(history) * 3.5 / 60, 1),  # rough estimate
        },
