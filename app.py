...
def player(song_id):
    ...
    # If local DB song has no stream URL, search JioSaavn by title+artist
    if not song.get("url") and song.get("title") and song.get("title") != "Unknown Song":
        query = f"{song['title']} {song.get('artist', '')}"
        results = fetch_songs(query, 5)
        for r in results:
            norm = _normalize_song(r) if isinstance(r, dict) else None
            if norm and norm.get("url"):
                song["url"] = norm["url"]
                if not song.get("image") or song["image"] == "/static/images/default-album.png":
                    song["image"] = norm.get("image", song.get("image", ""))
                print(f"[player] Found stream via search for: {song['title']}")
                break
