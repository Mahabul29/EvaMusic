...
def profile():
    try:
        ...
        try:
            ...
            if coll:
                user_doc = coll.find_one({"user_id": user_id})
        except Exception as e:
        ...
        profile_data = {
            ...
            "avatar_url":   user_doc.get("avatar_url") or "/static/images/default-album.png",
            "social_links": {
...
def edit_profile():
    ...
    if user_doc:
        raw_prefs = user_doc.get("preferences", {})
    else:
        raw_prefs = {}

    profile_data = {
        "username":     user_doc.get("username", "EvaUser") if user_doc else "EvaUser",
        "display_name": user_doc.get("display_name") or (user_doc.get("username", "EvaUser") if user_doc else "EvaUser"),
        "email":        user_doc.get("email", "") if user_doc else "",
        "bio":          user_doc.get("bio", "Music lover 🎵") if user_doc else "Music lover 🎵",
        "avatar_url":   user_doc.get("avatar_url", "/static/images/default-album.png") if user_doc else "/static/images/default-album.png",
        "social_links": user_doc.get("social_links", {"instagram": "", "twitter": "", "youtube": "", "spotify": ""}) if user_doc else {"instagram": "", "twitter": "", "youtube": "", "spotify": ""},
        "preferences":  {
            "theme":            raw_prefs.get("theme", "dark"),
            "accent_color":     raw_prefs.get("accent_color", "#1DB954"),
            "notifications":    raw_prefs.get("notifications", True),
            "auto_play":        raw_prefs.get("auto_play", True),
            "text_size":        raw_prefs.get("text_size", "medium"),
            "download_quality": raw_prefs.get("download_quality", "high"),
        }
    }
    ...
    themes = {
        "dark":   {"name": "Dark",   "bg": "#121212", "text": "#ffffff"},
        "light":  {"name": "Light",  "bg": "#ffffff", "text": "#121212"},
        "amoled": {"name": "AMOLED", "bg": "#000000", "text": "#ffffff"},
    }
...
def api_update_profile():
    ...
    user["updated_at"] = db.datetime.now(db.timezone.utc).isoformat()
