...
def artist_page(name):
    """Show all songs for a given artist name."""
    songs = []
    raw_results = fetch_songs(name, 30)
    for item in raw_results:
        norm = _normalize_song(item)
        if norm and norm.get("url"):
            # Keep only results that actually feature this artist
            if name.lower() in (norm.get("artist") or "").lower():
                songs.append(norm)
    if not songs:
        # Fall back to unfiltered search results rather than showing nothing
        for item in raw_results:
            norm = _normalize_song(item)
            if norm and norm.get("url"):
                songs.append(norm)
